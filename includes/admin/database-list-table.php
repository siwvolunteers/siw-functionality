...
class Database_List_Table extends \WP_List_Table {
	...
	public function __construct( protected Table $database_table ) {
		$this->query = $database_table->get_query();
		parent::__construct(
			[
				'singular' => $this->query->item_name,
				'plural'   => $this->query->item_name_plural,
			]
		);
	}
	...
	public function get_records( int $per_page, int $page_number = 1 ): ?array {
		...
		$args = [
			...
			'search'  => isset( $_REQUEST['s'] ) && ! empty( $_REQUEST['s'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['s'] ) ) : '', // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function column_default( $item, $column_name ): string {
		return isset( $item->$column_name ) ? (string) $item->$column_name : ''; // TODO: aparte weergave voor booleans en eventueel andere datatypes
	}
	...
	public function get_columns(): array {
		$all_columns = $this->database_table->get_schema()->get_columns();
		$columns = $this->database_table->get_view_columns();
	...
	public function get_sortable_columns(): array {
		$columns = $this->database_table->get_schema()->get_columns();
		$columns = wp_list_filter( $columns, [ 'sortable' => true ] );
	...
	public function prepare_items() {
		$per_page     = $this->get_items_per_page( $this->database_table->name . '_records_per_page', self::DEFAULT_ITEMS_PER_PAGE );
		$current_page = $this->get_pagenum();
		$total_items  = $this->database_table->count(); // Klopt niet, je moet het aantal records na filtering tellen
