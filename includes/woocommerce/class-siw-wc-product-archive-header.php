...
class SIW_WC_Product_Archive_Header {
	...
	public function add_archive_description() {
		...
		if ( false === $this->show_archive_header() ) {
			return;
		}

		$text = SIW_Formatting::array_to_text(
			array_filter(
				[
					$this->get_intro_text(),
					$this->get_sale_text(),
	...
	protected function get_teaser_text() {
		...
		if ( false === $this->is_teaser_text_active() ) {
			return null;
		}
	...
	protected function get_sale_text() {
		...
		if ( false ==  SIW_Util::is_workcamp_sale_active() ) {
			return null;
		}
